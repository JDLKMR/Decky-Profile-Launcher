...
DEFAULTS = {
    ...
    "variable": "PROFILE",
    # Display names. The value written to the script is the 1-based index,
    # so this list's length is the profile count. Rename/add/remove freely.
    ...
    "backup": True,
    # appid (as string) -> {
    #   "name": str, "script": str, "enabled": bool,
    #   "lastProfile": int, "detected": bool,
    #   "allowedProfiles": list[int],  # empty/missing = every profile applies
...
class Plugin:
    ...
    async def _main(self):
        ...
        decky.logger.info("Launch Profiles backend ready (%s games known)",
    ...
    def _load(self):
        ...
        if not merged.get("profiles"):
            merged["profiles"] = list(DEFAULTS["profiles"])
        return merged
    ...
    async def resolve_script(self, exe: str = "", launch_options: str = ""):
        ...
        if result["exists"]:
            info = await self.read_profile(chosen)
            result["hasVariable"] = info["hasVariable"]
